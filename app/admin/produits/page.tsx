...
export default async function Produits() {
  const result = await productService.list({ perPage: 100 }).catch(() => ({
    items: [], total: 0, totalPages: 0, page: 1,
  }));
  const products = result.items;
  const totals = {
    stock: products.reduce((s, p) => s + (p.stockQuantity ?? 0), 0),
    outOfStock: products.filter((p) => !p.inStock).length,
    revenuePotential: products.reduce((s, p) => s + (p.stockQuantity ?? 0) * p.price, 0),
  };
